...
Prediction Variable: Let's first analyse the prediction variable alone
```{r}
...
On the 300k datapoints we have, one third of them are no Show Datapoints while the remaining part is show. 

Date tof the data that we have
```{r}
dataset %>% 
...
```
2 Years data
Number of appointments is around constant
