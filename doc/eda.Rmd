...
Quick exploration of the data first
```{r}
...
Now, to explain a little bit how time influence our variable of interest, we show the evolution of the noshow ratio through the period we are studying.
...
Using a daily granularity, we observe a high variance that make the graph difficult to interpret. Let's change for a weekly granularity.
...
qplot(weekday_appointment, ratio, data = by_day, geom = c("boxplot"), xlab = "")
```

We clearly see a trend here: The ratio is extremely high on Saturday! It jumps from 30 to 37%.  

Let us have a look ot the moment in the day the patient make the appointment.
```{r}
...
g = ggplot(data = by_hour[by_hour$hour_registration>5,][by_hour$hour_registration<21,], aes(x = hour_registration, y = ratio)) + geom_line() + ylim(0,0.5) + xlab("Time") + ylab("Ratio")
g
```
...
Another important aspect in the time dimention is the time difference between the moment they make the appointment and the appointment date. 
```{r}
...
g = ggplot(data = by_diff, aes(x = daydiff_regist_appt, y = ratio)) + geom_line() + xlab("Day Difference") + ylab("Ratio")
g
```

We first notice that the curve is extremely heratic after a certain value due to the number of datapoint. Let us zoom in the 0-120 days window. 
```{r}
by_diff = group_by(dataset, daydiff_regist_appt)
...
```{r}
by_precipitation_inches = group_by(dataset, appointment_date)
by_precipitation_inches = summarise(by_precipitation_inches, show = sum(show_up==1), noshow = sum(show_up == 0), max_precipitation_inches = max(precipitation_inches), mean_precipitation_inches = mean(precipitation_inches))
by_precipitation_inches$ratio = by_precipitation_inches$noshow / (by_precipitation_inches$show+by_precipitation_inches$noshow)

qplot( max_precipitation_inches, ratio, data = by_precipitation_inches[by_precipitation_inches$max_precipitation_inches<2,]) + geom_smooth(show.legend = TRUE,method = 'lm')

summary(lm(ratio~max_precipitation_inches, data = by_precipitation_inches[by_precipitation_inches$max_precipitation_inches<2,]))
```

```{r}
by_temp = group_by(dataset, appointment_date)
by_temp = summarise(by_temp, show = sum(show_up==1), noshow = sum(show_up == 0), max_temp = max(max_temp_faren),lowest_visibility = max(as.numeric(lowest_visibility_miles)), lowest_visibility = max(as.numeric(lowest_visibility_miles)), wind = max(max_wind_mph), max_precipitation_inches = max(precipitation_inches))
by_temp$ratio = by_temp$noshow / (by_temp$show + by_temp$noshow)

qplot( max_temp, ratio, data = by_temp) + geom_smooth(show.legend = TRUE,method = 'lm')

summary(lm(ratio ~ max_temp, data = by_temp))
```

```{r}
by_visibility = group_by(dataset, appointment_date)
by_visibility = summarise(by_visibility, show = sum(show_up==1), noshow = sum(show_up == 0), lowest_visibility = max(as.numeric(lowest_visibility_miles)))
by_visibility$ratio = by_visibility$noshow / (by_visibility$show + by_visibility$noshow)

qplot( lowest_visibility, ratio, data = by_visibility) + geom_smooth(show.legend = TRUE,method = 'lm')

summary(lm(ratio ~ lowest_visibility, data = by_visibility))
```

```{r}
qplot( wind, ratio, data = by_temp) + geom_smooth(show.legend = TRUE,method = 'lm')

summary(lm(ratio ~ wind, data = by_temp))
```

```{r}
summary(lm(ratio ~ wind + lowest_visibility + max_temp + max_precipitation_inches, data = by_temp))
```
