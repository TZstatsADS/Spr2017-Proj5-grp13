...
We see that the polynomial model fit pretty well this part of the curve with 2 p-values extremely satisfactory. 
...
qplot( max_precipitation_inches, ratio, data = by_temp[by_temp$max_precipitation_inches<2,]) + geom_smooth(show.legend = TRUE,method = 'lm')

summary(lm(ratio~max_precipitation_inches, data = by_temp[by_temp$max_precipitation_inches<2,]))
```

We note that there is no correlation between the ratio and the precipitation level.
...
Same result with the maximum temperature.

```{r}
qplot( lowest_visibility, ratio, data = by_temp) + geom_smooth(show.legend = TRUE,method = 'lm')
...
Just like the two first variables, the wind speed is not correlated with the nosow ratio. 
