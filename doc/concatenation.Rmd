...
# df.test$boost.pred <- log(boost.pred)

ThresholdsList = seq(from = min(log(boost.pred)) , to = max(log(boost.pred)), by = 0.001)

rf.specificityList <- c()
boost.specificityList <- c()

for (i in 1:length(ThresholdsList)) {
  threshold <- ThresholdsList[i]
  
  pred.boost <- factor(ifelse(log(boost.pred) > threshold, 1, 0))
  boost.mat <- confusionMatrix(pred.boost, df.test$show_up)
  
  rf.boost <- factor(ifelse(log(boost.pred) > threshold, 1, 0))
  boost.mat <- confusionMatrix(pred.boost, df.test$show_up)
  ...
  # pred.rf     <- ifelse(log(boost.pred) > threshold, 1, 0)
  # rf.mat <- confusionMatrix(pred.rf, df.test$show_up)
  # rf.specificityList[i] <- specificity(rf.mat)
}

ThresholdsDF <- data.frame("ThresholdsList" = ThresholdsList,
                           # "RF.specificity" = rf.specificityList,
                           "Boost.specificity" = boost.specificityList)

# Opt.RF.threshold <- ThresholdsDF[which.max(ThresholdsDF$RF.specificity), "ThresholdsList")
Opt.Boost.threshold <- ThresholdsDF[which.max(ThresholdsDF$Boost.specificity), "ThresholdsList")
